...
module.exports = function getNamespacedClassName(localName, filePath) {
  ...
  const polarisComponentName =
    nestedComponentMatch && nestedComponentMatch.length > 1
      ? `${polarisVizClassName(nestedComponentMatch[1])}-${componentName}`
      : polarisVizClassName(componentName);
...
function polarisVizClassName(className) {
  return `PolarisViz-${className}`;
}
