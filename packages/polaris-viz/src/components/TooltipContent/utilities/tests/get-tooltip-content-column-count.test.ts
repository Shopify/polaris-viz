...
import {getTooltipContentColumnCount} from '../get-tooltip-content-column-count';

describe('getTooltipContentColumnCount()', () => {
  it('returns the default column count when data does not have a trend or color', () => {
    ...
    expect(getTooltipContentColumnCount(data)).toBe(2);
  });
  ...
  it('adds a column when data has a color', () => {
    ...
    expect(getTooltipContentColumnCount(data)).toBe(3);
  });
  ...
  it('returns all columns when data has a color and trend', () => {
    ...
    expect(getTooltipContentColumnCount(data)).toBe(4);
  });
});
